...
def calculate_grid_area_map(detection_result):
    ...
    if is_regular:
        ...
        dy_km = _haversine_vec(
            lat_2d[i_mid, j_mid],
            lon_2d[i_mid, j_mid],
            lat_2d[i_mid + 1, j_mid],
            lon_2d[i_mid + 1, j_mid],
        )
...
def prepare_grid_dict(ds):
    ...
    if lat_2d is None:
        ...
        raise ValueError(
            f"Could not determine grid coordinates from NetCDF. Found: {list(ds.coords)}"
        )
...
def calculate_kinematics(group):
    ...
    total_dist = sum(
        great_circle(coords[i], coords[i + 1]).km for i in range(len(coords) - 1)
    )
    ...
    return pd.Series(
        [total_dist, net_dist, straightness],
        index=["total_distance_km", "net_displacement_km", "track_straightness"],
    )
...
def calculate_area_change(group):
    ...
    if len(areas) < 2:
        return pd.Series(
            [0.0, 0.0], index=["max_area_volatility", "mean_area_volatility"]
        )
    ...
    return pd.Series(
        [max_vol, mean_vol], index=["max_area_volatility", "mean_area_volatility"]
    )
